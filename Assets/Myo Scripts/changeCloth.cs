...
public class changeCloth : MonoBehaviour
{
	...
	public void Start()
	{
		...
		clothSprite = Resources.LoadAll<Sprite> ("clothes");
		cloth = new GameObject ();
		cloth.AddComponent<SpriteRenderer> ();

		cloth.GetComponent<SpriteRenderer> ().sprite = clothSprite [0];
		cloth.transform.localScale = new Vector3 (0.19F, 0.19F, 0.19F); 
		cloth.transform.position += new Vector3 (-1.6F, 4.6F, 0.4F); 
		cloth.transform.Rotate(180F,90F,180F);
